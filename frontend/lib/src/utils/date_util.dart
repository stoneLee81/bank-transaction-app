...
class DateFormats {
  ...
  static String zh_y_mo_d_h_m = 'yyyy年MM月dd日 HH:mm';
...
class DateUtil {
  ...
  static String getWeekday(DateTime dateTime,
    ...
    if (ObjectUtils.isEmpty(dateTime)) return '';
    String weekday = '';
  ...
  static bool isToday(int milliseconds, {bool isUtc = false, int? locMs}) {
    if (ObjectUtils.isEmpty(milliseconds) || milliseconds == 0) return false;
    DateTime old =
  ...
  static bool isWeek(int ms, {bool isUtc = false, int? locMs}) {
    if ( ObjectUtils.isEmpty(ms) || ms <= 0) {
      return false;
    }
  ...
  static String getDayDiffDesc(DateTime datetime, {daydesc = const {0:'今天',1:'明天',2:'后天'}}) {
    String _desc;
    String _date = formatDate(datetime, format:DateFormats.y_mo_d);
    String _today = formatDate(DateTime.now(), format:DateFormats.y_mo_d);
    String _shortdate = formatDate(datetime, format:DateFormats.mo_d);
    int day = DateTime.parse(_date).difference(DateTime.parse(_today)).inDays; 

    _desc = ObjectUtils.isEmpty(daydesc[day]) ? ( day > 365 ? _date : _shortdate ) : daydesc[day];
